...
<<<<<<< HEAD
    try {
      ...
      const msg = data.alreadySubscribed
        ? `You're already subscribed! Here's a fresh ${data.percentOff}% code: ${data.code}`
        : `Welcome! Your ${data.percentOff}% code: ${data.code} (also sent by email) 🎉`
      toast.success(msg)
      setEmail("")
    } catch (err: any) {
